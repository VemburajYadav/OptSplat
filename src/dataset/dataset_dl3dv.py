...
class DatasetDL3DVCfg(DatasetCfgCommon):
    name: Literal["dl3dv_480p", "dl3dv_960p"]
    roots: list[Path]
        ...
        for chunk_path in self.chunks:
            ...
            for run_idx in range(int(times_per_scene * len(chunk))):
                ...
                for context_indices, target_indices in zip(c_list, t_list):
                    ...
                    if not torch.allclose(torch.det(extrinsics[context_indices][:, :3, :3]),
                                          torch.det(extrinsics[context_indices][:, :3, :3]).new_tensor(1)):
                        # print('invalid extrinsics')
                        continue
                    if not torch.allclose(torch.det(extrinsics[target_indices][:, :3, :3]),
                                          torch.det(extrinsics[target_indices][:, :3, :3]).new_tensor(1)):
                        # print('invalid extrinsics')
                        continue
