...
from .depth_predictor.multi_view_depth_transformer import DepthPredictorMultiView
...
@dataclass
class EncoderMultiViewDepthTransformerCfg:
    name: Literal["depth_transformer"]
    d_feature: int
    ...
    multi_view_depth_unet_feat_dim: int
    multi_view_depth_unet_channel_mult: List[int]
    multi_view_depth_unet_attn_res: List[int]
    random_world_origin_plucker_embeddings: bool # only valid during training
    depth_unet_feat_dim: int
    depth_unet_attn_res: List[int]
    depth_unet_channel_mult: List[int]
    unet_renderer_feat_dim: int
    unet_renderer_attn_res: List[int]
    wo_depth_refine: bool
    wo_backbone_cross_attn: bool
    wo_unet_renderer: bool
    use_epipolar_trans: bool
    wo_raw_gaussians_offset: bool


class EncoderMultiViewDepthTransformer(Encoder[EncoderMultiViewDepthTransformerCfg]):
    backbone: BackboneMultiview
    depth_predictor:  DepthPredictorMultiView
    gaussian_adapter: GaussianAdapter

    def __init__(self, cfg: EncoderMultiViewDepthTransformerCfg) -> None:
        super().__init__(cfg)
        ...
        # channels in the raw gaussian representation
        self.wo_raw_gaussians_offset = cfg.wo_raw_gaussians_offset
        self.raw_gaussian_offset_channels = 0 if cfg.wo_raw_gaussians_offset else 2
        self.gaussian_raw_channels = cfg.num_surfaces * (self.gaussian_adapter.d_in + self.raw_gaussian_offset_channels)
        ...
        self.depth_predictor = DepthPredictorMultiView(
            ...
            multi_view_depth_unet_feat_dim=cfg.multi_view_depth_unet_feat_dim,
            multi_view_depth_unet_channel_mult=tuple(cfg.multi_view_depth_unet_channel_mult),
            multi_view_depth_unet_attn_res=tuple(cfg.multi_view_depth_unet_attn_res),
            gaussian_raw_channels=self.gaussian_raw_channels,
            ...
            random_world_origin_plucker_embeddings=cfg.random_world_origin_plucker_embeddings
        )
    ...
    def forward(
        ...
        depths, densities, raw_gaussians = self.depth_predictor(
            ...
            target_extrinsics=target["extrinsics"] if target is not None else None
        )
        ...
        if self.wo_raw_gaussians_offset:
